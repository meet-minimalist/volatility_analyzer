...
class AnalysisVisualizer:
    ...
    def plot_single_stock_analysis(
        ...
        AnalysisVisualizer._plot_price_comparison(
            ...
            stock_data,
            benchmark_data,
            report.stock_metrics.name,
            report.benchmark_metrics.name,
        )
        ...
        # Plot 4: Rolling beta
        AnalysisVisualizer._plot_rolling_beta(axes[1, 1], report.rolling_metrics)

        # Plot 5: Scatter plot with regression
        AnalysisVisualizer._plot_scatter_regression(
            axes[2, 0],
            report.beta_analysis.aligned_data,
            report.stock_metrics.name,
            report.benchmark_metrics.name,
            report.beta_analysis.beta,
            report.beta_analysis.r_squared,
        )

        # Plot 6: R-squared over time
        AnalysisVisualizer._plot_r_squared(axes[2, 1], report.rolling_metrics)
    ...
    def _plot_price_comparison(
        ax, stock_data, benchmark_data, stock_name, benchmark_name
    ):
        ...
        ax.plot(
            norm_bench.index, norm_bench, label=benchmark_name, linewidth=2, alpha=0.7
        )
    ...
    def _plot_returns_distribution(
        ax, stock_returns, benchmark_returns, stock_name, benchmark_name
    ):
        """Plot returns distribution histogram"""
        ax.hist(stock_returns, bins=50, alpha=0.7, label=stock_name, density=True)
        ax.hist(
            benchmark_returns, bins=50, alpha=0.7, label=benchmark_name, density=True
        )
    ...
    def _plot_rolling_volatility(ax, rolling_vol):
        ...
        ax.axhline(
            y=mean_vol, color="black", linestyle="--", label=f"Mean: {mean_vol:.1f}%"
        )
        ax.fill_between(
            rolling_vol.index,
            rolling_vol,
    ...
    def _plot_scatter_regression(
        ax, aligned_data, stock_name, benchmark_name, beta, r_squared
    ):
        """Plot scatter plot with regression line"""
        ax.scatter(
            aligned_data["Benchmark"] * 100,
            aligned_data["Stock"] * 100,
            alpha=0.5,
            s=10,
        )
        ...
        ax.set_title(
            f"Scatter: Stock vs Benchmark Returns\nBeta = {beta:.2f}, R² = {r_squared:.2f}"
        )
        ax.legend()
        ax.grid(True, alpha=0.3)
    ...
    def plot_comparison(comparison_df: pd.DataFrame):
        ...
        AnalysisVisualizer._plot_volatility_comparison(
            axes[0, 0], stocks, comparison_df
        )
        ...
        plt.suptitle(
            "Multi-Stock Volatility & Beta Comparison", fontsize=16, fontweight="bold"
        )
        plt.tight_layout()
        plt.show()
    ...
    def _plot_volatility_comparison(ax, stocks, df):
        ...
        ax.axvline(
            x=median, color="blue", linestyle="--", label=f"Median: {median:.1f}%"
        )
        ax.legend()
    ...
    def _plot_volatility_ratio(ax, stocks, df):
        ...
        colors = [
            "red" if r > 1.5 else "orange" if r > 1.0 else "green" for r in ratios
        ]
        ...
        ax.axvline(x=1.0, color="black", linestyle="--", label="Same as Benchmark")
        ax.legend()
    ...
    def _plot_r_squared_comparison(ax, stocks, df):
        ...
        colors = [
            "green" if r > 50 else "orange" if r > 25 else "red" for r in r2_values
        ]
        bars = ax.barh(stocks, r2_values, color=colors)
        ax.set_xlabel("R-squared (%)")
        ax.set_title("Goodness of Fit (R-squared)")
        ax.set_xlim(0, 100)
        ax.axvline(x=50, color="blue", linestyle="--", label="50% explained by market")
        ax.legend()
